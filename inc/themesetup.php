...
function flexi_load_theme_fonts()
{
	$heading = get_theme_mod( 'flexi_google_fonts_heading_font' );
	$body = get_theme_mod( 'flexi_google_fonts_body_font' );
...
add_action( 'wp_head', 'flexi_load_theme_fonts' );

// load colors
function flexi_load_theme_colors()
{

	$bodyBackgroundColor = get_theme_mod( 'flexi_body_background_color', '#fff;' );
	$accentColor = get_theme_mod( 'flexi_accent_color', '#f0be0f' );
	$heroImageOverlayColor = get_theme_mod( 'flexi_hero_overlay_color', '#000' );
	$heroImageOverlayOpacity = get_theme_mod( 'flexi_hero_overlay_opacity', '80' );
	$heroImageBlur = get_theme_mod( 'flexi_hero_blur_enabled', '0' );
	$hero_image_bg = get_theme_mod( 'flexi_hero_bg_image' );
	echo '<style type="text/css">';
	...
	if ( !empty( $accentColor ) ) {
		...
		echo '.herotext a, .footerwidget a, h1.site-title, .postcontent a:hover, .authormeta a:hover, .post-edit-link:hover .sidebarwidget a:hover, .tab_cont .clear, .logged-in-as a:hover {border-color: ' . $hash . $accentColor . ' } ';
...
add_action( 'wp_head', 'flexi_load_theme_colors' );